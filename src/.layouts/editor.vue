<template>
    <div>
                ...
                <nav class="flex h-9 items-center justify-between" aria-label="Global">
                    ...
                    <div class="hidden lg:flex lg:min-w-0 lg:flex-1 lg:justify-center lg:gap-x-12">
                        <nuxt-link v-for="item in navigation" :key="item.name" :to="item.href"
                            class="font-semibold text-gray-900 hover:text-gray-900">{{ item.name }}</nuxt-link>
                    </div>
        ...
        <div class="uk-background-muted p-6" uk-height-viewport="offset-top: true; offset-bottom: true">
            <view-entry />
            <slot></slot>
        </div>
        <div class="uk-card uk-card-footer uk-card-default">
            ok
        </div>
    </div>
