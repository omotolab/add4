<template>
  <Renderer ref="renderer" pointer resize shadow alpha @click="randomColors">
...
import { toVideoBlobURL, toImageBlobURL } from 'assets/scripts/logic'
...
export default {
  ...
  methods: {
    ...
    async randomColors() {
        ...
        // const snapshot = await toImageBlobURL(elem.canvas)
        const recording = await toVideoBlobURL(elem.canvas, 5000)
        
        // this.$emit('snapshot', snapshot)
        this.$emit('recording', recording)
        // console.log('recording', recording)

        /* this.classifier.classify(elem.canvas, (err, results) => {
            console.log('err', err)
